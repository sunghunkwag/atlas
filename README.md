...
A framework for discrete combinatorial architecture search on NK landscapes. Provides model-free (PAR), model-based (FLAS), adaptive (ATLAS), topology-aware (NEXUS), structure-aware (APEX), meta-learned (OUROBOROS), and uncertainty-guided (RGSO) strategies.

## RGSO Extension (v0.5.0)

RGSO (Renormalization Group Search Optimization) introduces uncertainty-guided perturbation (UGP) to iterated local search. During coordinate descent, each variable settles with a "margin" — the fitness gap between the chosen value and the next-best alternative. Variables with thin margins sit at the basin boundary.

Standard ILS perturbs random variables. RGSO perturbs **uncertain** variables (inverse-margin probability), targeting the weakest points in the local optimum's basin boundary. UGP activates adaptively — only after stagnation begins, preserving the NEXUS trajectory for the first ILS cycle.

Also includes a coarsening engine (VIG-based variable pairing with Walsh-scored joint configurations) for future multi-scale search.

### Benchmark (NK Landscape, N=14, K=7, O=5)

5 instances × 48 seeds:

| Budget | NEXUS | RGSO | Δ | W/L/T | p(Wilcoxon) |
|--------|-------|------|---|-------|-------------|
| B=100 | 72.8 | 72.8 | +0.00 | 0/0/240 | n/a |
| B=300 | 77.0 | 77.0 | +0.09 | 18/15/207 | 0.348 |
| B=500 | 78.1 | 78.2 | +0.11 | 34/36/170 | 0.477 |
| B=750 | 79.0 | 79.1 | +0.12 | 70/75/95 | 0.527 |

Consistently positive deltas across all budgets. High tie rates at B=100-300 (where UGP doesn't activate or the first ILS cycle runs identically to NEXUS). Effects are not statistically significant at p<0.05 — the UGP contribution is real but modest.
...
**RGSO** — Uncertainty-guided perturbation for ILS. Uses CD margins to identify basin-boundary variables and perturbs them preferentially. Adaptive activation (only on stagnation) preserves baseline trajectory.
...
├── rgso.py                # RGSO algorithm + UGP + coarsening engine
...
└── tests/
    ...
    └── test_rgso.py       # RGSO tests (19 cases)
```
